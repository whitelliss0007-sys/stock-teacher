...
def get_fundamental_data(code):
    ...
    if code.isdigit():
        ...
        if is_etf:
            data['Type'] = 'ETF'
            data['Opinion'] = "ℹ️ ETF는 개별 기업이 아니므로 영업이익/PER 분석을 생략합니다."
            return data
...
def scan_undervalued_stocks():
    ...
    for i, stock in enumerate(target_stocks):
        ...
        try:
            ...
            reasons = []
            if 0 < per <= 15: reasons.append(f"💰 **쌉니다 (PER {per})**: 치킨집 본전 뽑는데 {per}년 걸림.")
            if 0 < pbr <= 1.2: reasons.append(f"🏗️ **안전빵 (PBR {pbr})**: 망해도 짐만 팔아도 본전.")
            if roe >= 10: reasons.append(f"👨‍🍳 **장사의 신 (ROE {roe}%)**: 사장님 실력이 훌륭함.")
            if 0 < psr < 1.0: reasons.append(f"🔥 **매출 대비 저평가 (PSR {psr})**: 시총이 연매출보다 작음.")
